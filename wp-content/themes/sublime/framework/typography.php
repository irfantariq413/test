...
// Start class
if ( ! class_exists( 'Sublime_Typography' ) ) {
	class Sublime_Typography {
		public function __construct() {
			// Customizer actions
			add_action( 'customize_register', array( 'Sublime_Typography' , 'register' ), 40 );

			// Loads Google fonts
			add_action( 'wp_enqueue_scripts', array( 'Sublime_Typography', 'load_fonts' ) );
		
			// CSS output
			if ( is_customize_preview() ) {
				add_action( 'customize_preview_init', array( 'Sublime_Typography', 'customize_preview_init' ) );
				add_action( 'wp_head', array( 'Sublime_Typography', 'live_preview_styles' ), 999 );
			} else {
				add_filter( 'sublime_custom_colors_css', array( 'Sublime_Typography', 'head_css' ), 999 );
			}
		}
		...
		public static function elements() {
			...
			// Return settings
			$array = apply_filters( 'sublime_typography_settings', array(
				'body' => array(
					'label' => esc_html__( 'Body', 'sublime' ),
					'target' => 'body',
					'defaults' => array(
				...
				'headings' => array(
					'label' => esc_html__( 'Headings', 'sublime' ),
				...
				'bottom_nav' => array(
					'label' => esc_html__( 'Bottom Menu', 'sublime' ),
					'target' => '#bottom .bottom-bar-menu a',
					'exclude' => array( 'font-color' ),
				),
				'entry_h1' => array(
					'label' => esc_html__( 'H1', 'sublime' ),
					'target' => 'h1',
				),
				'entry_h2' => array(
					'label' => esc_html__( 'H2', 'sublime' ),
					'target' => 'h2',
				),
				'entry_h3' => array(
					'label' => esc_html__( 'H3', 'sublime' ),
					'target' => 'h3',
				),
				'entry_h4' => array(
					'label' => esc_html__( 'H4', 'sublime' ),
					'target' => 'h4',
				),
				'woocommerce_product_title' => array(
					'label' => esc_html__( 'Woocommerce: Product Title', 'sublime' ),
					'target' => '.products li h2',
					'exclude' => array( 'font-color' ),
					'active_callback' => 'sublime_cac_has_woo',
				),
				'woocommerce_price' => array(
					'label' => esc_html__( 'Woocommerce: Price Amount', 'sublime' ),
					'target' => '.products li .price',
					'exclude' => array( 'font-color' ),
					'active_callback' => 'sublime_cac_has_woo',
				),
				'woocommerce_single_product_title' => array(
					'label' => esc_html__( 'Woocommerce: Single Product Title', 'sublime' ),
					'target' => '.woo-single-post-class .summary h1',
					'exclude' => array( 'font-color' ),
					'active_callback' => 'sublime_cac_has_woo',
				),
				'woocommerce_single_price' => array(
					'label' => esc_html__( 'Woocommerce: Single Price Amount', 'sublime' ),
		...
		public static function customize_preview_init() {
			wp_enqueue_script( 'sublime-typography-customize-preview',
				get_template_directory_uri() .'/framework/customizer/typography-customize.js',
				array( 'customize-preview' ),
				'1.0.0',
				true
			);
			wp_localize_script( 'sublime-typography-customize-preview', 'sublime', array(
		...
		public static function register ( $wp_customize ) {
			...
			// Add General Panel
			$wp_customize->add_panel( 'sublime_typography', array(
				'priority' => 142,
				'capability' => 'edit_theme_options',
				'title' => esc_html__( 'Typography', 'sublime' ),
			) );

			// Add General Tab
			$wp_customize->add_section( 'sublime_typography_general' , array(
				'title' => esc_html__( 'General', 'sublime' ),
				'priority' => 1,
				'panel' => 'sublime_typography',
			) );
			...
			$wp_customize->add_control( new Sublime_Customize_Multicheck_Control( $wp_customize, 'google_font_subsets', array(
				'label' => esc_html__( 'Font Subsets', 'sublime' ),
				'section' => 'sublime_typography_general',
			...
			foreach( $elements as $element => $array ) {
				...
				if ( $label ) {
					...
					// Define Section
					$wp_customize->add_section( 'sublime_typography_'. $element , array(
						'title' => $label,
						'priority' => $count,
						'panel' => 'sublime_typography',
					...
					if ( in_array( 'font-family', $attributes ) ) {
						...
						// Add Control
						$wp_customize->add_control( new Sublime_Fonts_Dropdown_Custom_Control( $wp_customize, $element .'_typography[font-family]', array(
								'label' => esc_html__( 'Font Family', 'sublime' ),
								'section' => 'sublime_typography_'. $element,
					...
					if ( in_array( 'font-weight', $attributes ) ) {
						$wp_customize->add_setting( $element .'_typography[font-weight]', array(
							'type' => 'theme_mod',
							'description' => esc_html__( 'Note: Not all Fonts support every font weight style.', 'sublime' ),
							'sanitize_callback' => false,
							'transport' => $transport,
						) );
						$wp_customize->add_control( $element .'_typography[font-weight]', array(
							'label' => esc_html__( 'Font Weight', 'sublime' ),
							'section' => 'sublime_typography_'. $element,
							...
							'choices' => array(
								...
								'100' => esc_html__( 'Extra Light: 100', 'sublime' ),
								'200' => esc_html__( 'Light: 200', 'sublime' ),
								'300' => esc_html__( 'Book: 300', 'sublime' ),
								'400' => esc_html__( 'Normal: 400', 'sublime' ),
								'500' => esc_html__( 'Medium: 500', 'sublime' ),
								'600' => esc_html__( 'Semibold: 600', 'sublime' ),
								'700' => esc_html__( 'Bold: 700', 'sublime' ),
								'800' => esc_html__( 'Extra Bold: 800', 'sublime' ),
								'900' => esc_html__( 'Black: 900', 'sublime' ),
							),
							'description' => esc_html__( 'Important: Not all fonts support every font-weight.', 'sublime' ),
						) );
					}
					...
					if ( in_array( 'font-style', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[font-style]', array(
							'label' => esc_html__( 'Font Style', 'sublime' ),
							'section' => 'sublime_typography_'. $element,
							...
							'choices' => array(
								'' => esc_html__( 'Default', 'sublime' ),
								'normal' => esc_html__( 'Normal', 'sublime' ),
								'italic' => esc_html__( 'Italic', 'sublime' ),
							),
						) );
					}
					...
					if ( in_array( 'text-transform', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[text-transform]', array(
							'label' => esc_html__( 'Text Transform', 'sublime' ),
							'section' => 'sublime_typography_'. $element,
							...
							'choices' => array(
								'' => esc_html__( 'Default', 'sublime' ),
								'capitalize' => esc_html__( 'Capitalize', 'sublime' ),
								'lowercase' => esc_html__( 'Lowercase', 'sublime' ),
								'uppercase' => esc_html__( 'Uppercase', 'sublime' ),
							),
						) );
					}
					...
					if ( in_array( 'font-size', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[font-size]', array(
							'label' => esc_html__( 'Font Size', 'sublime' ),
							'section' => 'sublime_typography_'. $element,
							'settings' => $element .'_typography[font-size]',
							'priority' => 5,
							'type' => 'text',
							'description' => esc_html__( 'Value in px or em.', 'sublime' ),
							'active_callback' => $active_callback,
						) );
					}
					...
					if ( in_array( 'font-color', $attributes ) ) {
						...
						$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, $element .'_typography_color', array(
							'label' => esc_html__( 'Font Color', 'sublime' ),
							'section' => 'sublime_typography_'. $element,
					...
					if ( in_array( 'line-height', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[line-height]',
							array(
								'label' => esc_html__( 'Line Height', 'sublime' ),
								'section' => 'sublime_typography_'. $element,
					...
					if ( in_array( 'letter-spacing', $attributes ) ) {
						...
						$wp_customize->add_control( new WP_Customize_Control( $wp_customize, $element .'_typography_letter_spacing', array(
							'label' => esc_html__( 'Letter Spacing', 'sublime' ),
							'section' => 'sublime_typography_'. $element,
		...
		public static function loop( $return = 'css' ) {
			...
			foreach( $elements as $element => $array ) {
				...
				$get_mod = sublime_get_mod( $element .'_typography' );
		...
		public static function load_fonts() {
			...
			if ( ! empty( $fonts ) && is_array( $fonts ) ) {
				foreach ( $fonts as $font ) {
					sublime_enqueue_google_font( $font );
				}
			}

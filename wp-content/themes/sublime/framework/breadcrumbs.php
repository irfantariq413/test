...
function sublime_breadcrumbs() {

    global $post;

    $home_text     = esc_html__( 'Home','sublime' );
    $category_text = esc_html__( 'Archive by Category "%s"','sublime' );
    $tax_text      = esc_html__( 'Archive by "%s"','sublime' );
    $tag_text      = esc_html__( 'Posts Tagged "%s"','sublime' );
    $author_text   = esc_html__( 'Articles Posted by %s','sublime' );
    $error_text    = esc_html__( 'Error 404','sublime');
    $search_text   = esc_html__( 'Search Results for "%s" Query','sublime' );

    $blog_text     = esc_html__( 'Blog','sublime' );
    $project_text  = esc_html__( 'Projects','sublime' );
    $product_text  = esc_html__( 'Shop','sublime' );
    $service_text  = esc_html__( 'Service','sublime' );
            ...
            foreach ( $cpt_list as $cpt ) {
                ...
                if ( $cpt->name == 'project' ) {
                    if ( is_singular( 'project' ) ) {
                        ...
                        if ( $page_id = sublime_parse_obj_id( sublime_get_mod( 'portfolio_page' ), 'page' ) ) {
                    ...
                    if ( is_singular( 'service' ) ) {
                        ...
                        if ( $page_id = sublime_parse_obj_id( sublime_get_mod( 'service_page' ), 'page' ) ) {
                            $page_permalink = get_permalink( $page_id );
                            $page_name      = get_the_title( $page_id );
                            if ( $page_permalink && $page_name ) {
                                echo '<a href="'. esc_url( $page_permalink ) .'" title="'. esc_attr( $page_name ) .'">'. esc_html( $page_name ) .'</a>';

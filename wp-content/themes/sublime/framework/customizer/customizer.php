...
// Start class
if ( ! class_exists( 'Sublime_Customizer' ) ) {
	class Sublime_Customizer {
		...
		public function __construct() {
			...
			$this->panels = array(
				'general' => array(
					'title' => esc_html__( 'General Options', 'sublime' )
				),
				'header' => array(
					'title' => esc_html__( 'Header', 'sublime' )
				),
				'featuredtitle' => array(
					'title' => esc_html__( 'Featured Title', 'sublime' )
				),
				'maincontent' => array(
					'title' => esc_html__( 'Main Content', 'sublime' )
				),
				'layout' => array(
					'title' => esc_html__( 'Layout', 'sublime' )
				),
				'blog' => array(
					'title' => esc_html__( 'Blog', 'sublime' )
				),
				'blogsingle' => array(
					'title' => esc_html__( 'Blog Single Post', 'sublime' )
				),
				'sidebarwidget' => array(
					'title' => esc_html__( 'Sidebar Widget', 'sublime' )
				),
				'shop' => array(
					'title' => esc_html__( 'Shop', 'sublime' )
				),
				'footer' => array(
					'title' => esc_html__( 'Footer', 'sublime' )
				),
				'footerwidget' => array(
					'title' => esc_html__( 'Footer Widget', 'sublime' )
				),
				'bottombar' => array(
					'title' => esc_html__( 'Bottom Bar', 'sublime' )
				),
			);
			...
			if ( ! is_admin() || is_customize_preview() ) {
				...
				if ( is_customize_preview() && $this->enable_postMessage ) {
					...
					add_filter( 'sublime_custom_colors_css', array( $this, 'head_css' ), 999 );
				}
			}
		}
		...
		public function add_sections() {
			...
			// Apply filters
			$this->sections = apply_filters( 'sublime_customizer_sections', $this->sections );
		}
		...
		public function add_customizer_panels_sections( $wp_customize ) {
			...
			foreach( $all_panels as $id => $val ) {
				...
				if ( $is_section ) {
					...
					$wp_customize->add_section( 'sublime_'. $id, array(
						'priority' => $priority,
						'title'    => $title
					) );
				}
				...
				// Add Panel
					...
					$wp_customize->add_panel( 'sublime_'. $id, array(
						'priority' => $priority,
						'title'    => $title
					) );
				}
		...
		public function create_sections( $wp_customize ) {
			...
			foreach ( $this->sections as $section_id => $section ) {
				...
				foreach ( $settings as $setting ) {
					...
					if ( 'sublime-heading' == $control_type ) {
					...
					} elseif ( 'sublime-heading' == $control_type ) {
						$control_object = 'Sublime_Customizer_Heading_Control';
					} elseif ( 'sublime-sortable' == $control_type ) {
						$control_object = 'Sublime_Customize_Control_Sorter';
					} elseif ( 'sublime_textarea' == $control_type ) {
						$control_object = 'Sublime_Customizer_Textarea_Control';
					} else {
		...
		public static function customize_controls_print_styles() { ?>
			 <style type="text/css">
				/* Icons */
				#accordion-panel-sublime_general > h3:before,
				#accordion-panel-sublime_typography > h3:before,
				#accordion-panel-sublime_topbar > h3:before,
				#accordion-panel-sublime_header > h3:before,
				#accordion-panel-sublime_featuredtitle > h3:before,
				#accordion-panel-sublime_maincontent > h3:before,
				#accordion-panel-sublime_layout > h3:before,
				#accordion-panel-sublime_blog > h3:before,
				#accordion-panel-sublime_blogsingle > h3:before,
				#accordion-panel-sublime_sidebarwidget > h3:before,
				#accordion-panel-sublime_projects > h3:before,
				#accordion-panel-sublime_shop > h3:before,
				#accordion-panel-sublime_footer > h3:before,
				#accordion-panel-sublime_footerwidget > h3:before,
				#accordion-panel-sublime_bottombar > h3:before { display: inline-block; width: 20px; height: 20px; font-size: 18px; line-height: 24px; font-style: normal; vertical-align: top; text-align: center;-webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; color: #0085ba; margin-right: 6px; font-family: "dashicons"; content: ""; }

				#accordion-panel-sublime_general > h3:before { content: "\f108" }
				#accordion-panel-sublime_typography > h3:before { content: "\f215" }
				#accordion-panel-sublime_topbar > h3:before { content: "\f480" }
				#accordion-panel-sublime_header > h3:before { content: "\f175" }
				#accordion-panel-sublime_featuredtitle > h3:before { content: "\f209" }
				#accordion-panel-sublime_maincontent > h3:before { content: "\f134" }
				#accordion-panel-sublime_layout > h3:before { content: "\f535" }
				#accordion-panel-sublime_blog > h3:before { content: "\f109" }
				#accordion-panel-sublime_blogsingle > h3:before { content: "\f109" }
				#accordion-panel-sublime_sidebarwidget > h3:before { content: "\f135" }
				#accordion-panel-sublime_projects > h3:before { content: "\f481" }
				#accordion-panel-sublime_shop > h3:before { content: "\f174" }
				#accordion-panel-sublime_footerwidget > h3:before { content: "\f209" }
				#accordion-panel-sublime_footer > h3:before { content: "\f535" }
				#accordion-panel-sublime_footer > h3:before { content: "\f535" }
				#accordion-panel-sublime_bottombar > h3:before { content: "\f138" }
				
				/* General Tweaks */
				.customize-control-select select { min-width: 100% }

				/* Slider UI */
				li.customize-control.customize-control-sublime_slider_ui input[type=text] { width: 20%; float: left; text-align: center; }
				li.customize-control.customize-control-sublime_slider_ui .ui-slider-horizontal.sublime-slider-ui { float: right; width: 75%; height: 5px; margin-top: 10px; color: #414042; position: relative; border-radius: 3px; border: 1px solid #747474; border-bottom-color: #aeaeae; background-color: #cdcdcd; background: -webkit-linear-gradient(#aaaaaa, #cdcdcd); background: -o-linear-gradient(#aaaaaa, #cdcdcd); background: -moz-linear-gradient(#aaaaaa, #cdcdcd); background: linear-gradient(#aaaaaa, #cdcdcd); }
				...
				/* Sortable */
				.sublime-sortable ul { margin-top: 10px }
				.sublime-sortable li.sublime-sortable-li { cursor: move; background: #fff; padding: 0; padding-left: 15px; height: 40px; line-height: 40px; white-space: nowrap; border: 1px solid #dfdfdf; text-overflow: ellipsis; -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none; user-select: none; color: #222; margin-bottom: 5px; margin-top: 0; overflow: hidden; position: relative; }
				.sublime-sortable li.sublime-sortable-li:hover { border-color: #999; z-index: 10; }
				.sublime-sortable li.sublime-sortable-li .fa { display: block; position: absolute; top: 0; right: 0; width: 42px; height: 40px; line-height: 40px; margin: 0; text-align: center; font-size: 18px; }
				.sublime-sortable li.sublime-sortable-li .fa:after{font-family:dashicons;font-size:20px;position:absolute;left:0;top:0;content:"\f147";color:#0085ba;width:100%;text-align:center}
				.sublime-sortable li.sublime-sortable-li .fa:before{content:"";position:absolute;right:6px;top:6px;width:26px;height:26px;background-color:#fff;border:1px solid #dfdfdf}
				.sublime-sortable li.sublime-sortable-li .fa:hover:before { border-color: #0085ba; }
				.sublime-sortable li.sublime-sortable-li .sublime-hide-sortee { cursor: pointer }
				.sublime-sortable ul li:last-child { margin-bottom: 0 }
				.sublime-sortable li.sublime-hide .fa:after { color: #dfdfdf; }
				.sublime-sortable li.sublime-hide { opacity: 0.65; }

				/* Custom Heading */
				.sublime-customizer-heading { color: #23282d; display: block; padding-top: 28px; padding-bottom: 5px; border-bottom: 2px solid #ddd; font-size: 16px; font-weight: 500; }
			 </style>
		...
		public function customize_preview_init() {
			if ( $this->enable_postMessage ) {
				wp_enqueue_script( 'sublime-customize-preview',
		...
		public function loop_through_inline_css( $return = 'css' ) {
			...
			foreach ( $settings as $settings_array ) {
				foreach ( $settings_array as $setting ) {
					...
					$theme_mod  = sublime_get_mod( $setting_id, $default );
					...
					if ( apply_filters( 'sublime_remove_default_mods', false ) ) {
						$get_all_mods = sublime_get_mods();
					...
					// Sanitize data
					if ( $sanitize ) {
						$theme_mod = sublime_sanitize_data( $theme_mod, $sanitize );
					} else {
						$theme_mod = $theme_mod;
					}
